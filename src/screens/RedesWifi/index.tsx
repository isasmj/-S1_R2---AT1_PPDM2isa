...
const typeLavels: Record<string, string> = {
    WIFI: 'wi-fi',
...
export default function RedesWifiScreen() {
    ...
    const carregarRede = useCallback(async () => {
        ...
        try {
            ...
            console.error(error)
            setErrorMsg("Não foi possível obter informações da rede")
        } finally {
            setLoading(false)
        }
    }, []);
    ...
    return (
        <View>
            ...
            <Text style={styles.subtitle}>Status da conexão ativa no dispositivo</Text>
                ...
                info && (
                    <View style={styles.card}>
                        <View style={styles.statusRow}>
                            <View style={[styles.statusDot, { backgroundColor: info.isConnected ? '#25993e' : '#B12727' }]}>
                            </View>

                        </View>
                        <Text style={styles.infoRow}>Tipo de conexão</Text>
                        <Text style={styles.infoValue}>{isWifi ? 'Conectado por Wi-fi' : 'Não conectado por Wi-fi'}</Text>

                        <Text style={styles.infoRow}>Endereço do IP</Text>
                        <Text style={styles.infoValue}>{info.ipAddres}</Text>

                        <Text style={styles.infoRow}>Status modo avião</Text>
                        <Text style={styles.infoValue}>{info.isAirplaneMode ? 'Ativado' : 'Desativado'}</Text>
                    </View>
                )
            )}
